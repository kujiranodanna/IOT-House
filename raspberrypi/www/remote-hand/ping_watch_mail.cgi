...
<TITLE>Ping monitoring and e-mail in the configuration</TITLE>
...
<HR>
<TABLE ALIGN=RIGHT><TR><TD>&copy;20-2022 pepolinux.com</TD><TR></TABLE>
</BODY>
</HTML>'

CMD=/www/remote-hand/tmp/ping_watch_mail.pepocmd
PING_MAIL=/www/remote-hand/tmp/.ping_mail_list
...
for n in 0 1 2 3 ; do
  if [ -n "${ip[$n]}" ] && [ "${reg[$n]}" = "reg" ] && [ -n "${mail[$n]}" ];then
   if [ -e "$PING_MAIL" ];then
      cat "$PING_MAIL" | grep -F -v "${ip[$n]}" > "$PING_TMP"
      mv "$PING_TMP" "$PING_MAIL"
    fi
    echo "$PING_WATCH" "${ip[$n]}" "${ip_alias[$n]}" "${mail[$n]}" "${interval[0]}" >> "$PING_MAIL"
    if [ -e "$PING_CRON" ];then
    ...
    cat >> "$PING_CRON" <<END
0-59/${interval[0]} * * * * $PING_WATCH ${ip[$n]} ${mail[$n]} ${ip_alias[$n]}
END
  fi
  if [ "${reg[$n]}" = "del" ];then
    if [ -e "$PING_CRON" ];then
