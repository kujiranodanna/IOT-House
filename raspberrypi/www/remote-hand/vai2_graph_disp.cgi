...
<META http-equiv="Refresh" content="8;URL=/remote-hand/tmp/i2c_vai2.png">
...
<TR ALIGN=CENTER class="blink"><TD>I2C Analog input-2 graph create</TD></TR>
</TABLE>
<HR>
...
GRAP_TEMPFILE=${WORKDIR}/tmp/i2c_vai2.png
END=`date "+%Y%m%d %H:%M:00" -d "-10 minute"`
END=`date -d "${END}" +%s`
CMD=${WORKDIR}/tmp/i2c_vai2.pepocmd
if [ -e ${DSFILE} ];then
  [ -e ${RRDSTART} ] && . ${RRDSTART}
...
cat >${CMD}<<EOF
#!/bin/bash
rrdtool graph ${GRAP_TEMPFILE} --width 600 --height 120 --start ${START} --end ${END} --upper-limit 2000 --lower-limit 0 DEF:a=${DSFILE}:vai2:MAX LINE1:a#FF0000:"I2C Analog input-2"
EOF
fi
